...
class ESGMonitor:
    ...
    def check_reuters_legal(self):
        """Monitor Reuters Legal for major enforcement cases ONLY"""
        print("Checking Reuters Legal (strict filter)...")
        new_items = []
        ...
        try:
            ...
            for entry in feed.entries[:20]:
                ...
                has_enforcement = any(term in combined for term in [
                    'lawsuit', 'sues', 'sued', 'settlement', 'court', 'judge',
                    'injunction', 'blocks', 'ruling', 'eeoc files', 'sec charges'
                ])
                
                # Must have ESG/DEI keywords
                has_esg = any(term in combined for term in [
                    'eeoc', 'discrimination', 'climate', 'esg', 'sustainability',
                    'accessibility', 'ada', 'diversity', 'pay equity',
                    'sb 253', 'sb 261', 'csrd'
                ])
                
                # STRICT: Must be actual regulatory/legal action, not commentary
                if has_enforcement and has_esg:
                    # Block marketing/commentary phrases
                    is_commentary = any(phrase in combined for phrase in [
                        'what companies need to know', 'what employers need to know',
                        'countdown to', 'guide to', 'how to', 'prepare for'
                    ])
                    
                    if not is_commentary and not any(r['title'] == title for r in self.regulations):
                        pub_date = entry.get('published', entry.get('updated', ''))
                        parsed_date = self.parse_date(pub_date)
                        ...
                        # Determine category
                        category = 'environmental'
                        if any(term in combined for term in ['eeoc', 'discrimination', 'diversity', 'ada', 'accessibility']):
                            category = 'social'
                        ...
                        regulation = {
                            ...
                            'jurisdiction': 'federal' if category == 'social' else 'international',
                            'type': 'enforcement',
                            'priority': 'high',
                            'date': parsed_date,
                            'isNew': True,
    ...
    def send_email_alert(self, new_regulations):
        ...
        try:
            # Group by category
            env_regs = [r for r in new_regulations if r.get('category') == 'environmental']
            social_regs = [r for r in new_regulations if r.get('category') == 'social']
            
            # Create HTML email
            html_body = f"""
            <html>
            <body style="font-family: Arial, sans-serif;">
                <h2>🚨 ESG Regulations Alert</h2>
                <p><strong>{len(new_regulations)} new regulations detected</strong></p>
            ...
            msg['Subject'] = f'🚨 ESG Alert: {len(new_regulations)} New Regulations'
            msg['From'] = EMAIL_FROM
            msg['To'] = EMAIL_TO
