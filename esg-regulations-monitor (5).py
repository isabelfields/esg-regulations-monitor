...
class RegulationMonitor:
    ...
    def is_esg_related(self, text):
        ...
        exclusions = [
            ...
            # Educational content (but keep regulatory analysis from law schools)
            'certificate program', 'continuing education', 'webinar registration',
            ...
            # Commentary about past actions (not new regulations)
            'eliminated', 'removed', 'scrapped', 'rolled back', 'reversed',
            'other governments are', 'doing the opposite', 'in contrast',
            # Explainer/educational content (not regulatory updates)
            'what is', 'what are', 'what does', 'what the', 'understanding', 'explained', 'explained:',
            'primer on', 'introduction to', 'basics of', 'overview of',
            'everything you need to know', 'complete guide', 'beginner\'s guide',
            'navigating', 'scaling the', 'are you ready', 'what it means',
            'what companies should know', 'hosts', 'to discuss', 'summary:',
            ...
            # Law firm marketing (only specific phrases, not all law firm content)
            'what it means for employers', 'implications for employers', 
            'key takeaways for', 'employers should know', 'practical guidance for clients',
        ...
        for feed_url, source_type in feeds:
            try:
                ...
                for entry in feed.entries[:20]:  # Check last 20 items
                    ...
                    # Must be ESG-related AND have regulatory action
                    has_action_verb = any(term in combined for term in [
                        ...
                        # Specific regulation mentions
                        'rule', 'regulation', 'law', 'statute', 'directive'
                    ])
                    
                    # If it has specific regulation names, still need some regulatory context
                    has_strong_esg_term = any(term in combined for term in [
                        'sb 253', 'sb 261', 'sb253', 'sb261',
                        'csrd', 'issb', 'ifrs s1', 'ifrs s2'
                    ])
                    
                    # Need action verb OR (strong term + regulatory context)
                    is_regulatory = has_action_verb or (has_strong_esg_term and any(word in combined for word in ['rule', 'regulation', 'law', 'guidance', 'standard']))
    ...
    def check_dei_news(self):
        """Monitor Reuters and targeted EEOC news for DEI enforcement"""
        print("Checking Reuters and EEOC enforcement news...")
        new_items = []
        
        # Reuters feeds + targeted EEOC Google News
        news_sources = [
            ('https://www.reuters.com/rssfeed/legal', 'Reuters Legal'),
            ('https://www.reuters.com/rssfeed/companyNews', 'Reuters Business'),
            ('https://www.reuters.com/rssfeed/sustainability', 'Reuters Sustainability'),
            # Targeted search for EEOC enforcement only (very specific)
            ('https://news.google.com/rss/search?q=EEOC+lawsuit+OR+settlement+OR+investigation+-blog+-webinar&hl=en-US&gl=US&ceid=US:en', 'Google News - EEOC Cases'),
        ]
        
        # DEI enforcement keywords
        dei_keywords = [
            'eeoc', 'discrimination', 'equal employment',
            'pay transparency', 'pay equity', 'board diversity',
            'dei', 'diversity mandate', 'harassment lawsuit',
        ...
        for feed_url, source_name in news_sources:
            try:
                ...
                for entry in feed.entries[:20]:
                    ...
                    # Must have DEI/enforcement keywords
                    if any(keyword in combined for keyword in dei_keywords):
                        # Must also have enforcement/regulatory context
                        has_regulatory = any(term in combined for term in [
                            'lawsuit', 'settlement', 'investigation', 'investigates', 'sues', 'sued',
                            'charges', 'fine', 'penalty', 'court', 'judge',
                            'law passed', 'bill passed', 'regulation', 'mandate',
                            'eeoc files', 'doj announces', 'opens investigation', 'probes'
                        ])
                        
                        if has_regulatory and not any(r['title'] == title for r in self.regulations):
                            pub_date = entry.get('published', entry.get('updated', ''))
                            parsed_date = self.parse_date(pub_date)
                            ...
                            if 'california' in combined or 'state' in combined:
                            ...
                            regulation = {
                                ...
                                'title': title,
                                'category': 'social',
                                'source_category': 'dei-enforcement',
                                'jurisdiction': jurisdiction,
                                'type': 'enforcement',
                                'priority': 'high' if 'lawsuit' in combined or 'settlement' in combined else 'medium',
                                'date': parsed_date,
                                'isNew': True,
                                'description': description[:500] + ('...' if len(description) > 500 else ''),
                                'tags': ['DEI', 'Enforcement'],
                                'effectiveDate': 'TBD',
                                'source_url': entry.get('link', ''),
                                'source_type': source_name
                            }
